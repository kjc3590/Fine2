...
jQuery(window).load(function(){
	...
	console.log($('.filter-wrapper li').attr("data-filter")=='HOSPITAL');
   ...
   jQuery('.btn_load_more').click(function(){
      ...
      if (now_load > 0) {
         if (addon_options.type == 0) {
            ...
            for (i = i_start-1; i < i_start+now_load-1; i++) {
               loaded_object = loaded_object + '<div class="iso-box '+ addon_options.items[i].classname +' element col-md-3 col-sm-3"><div class="portfolio-thumb"><img src="'+addon_options.items[i].src+'" class="fluid-img" alt="portfolio"><div class="portfolio-overlay"><div class="portfolio-item"><h4>"'+addon_options.items[i].content+'"</h4><a href="'+addon_options.items[i].imgurl+'" id="tttt" data-lightbox-gallery="'+addon_options.items[i].data+'"><i class="fa fa-link"></i></a><a href="'+addon_options.items[i].url+'" target="_blank"><i class="fa fa-share" id="tt"></i></a></div></div></div></div>';
